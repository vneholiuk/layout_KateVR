...
function updateAboutSlider(slideIndex) {
    ...
    aboutSliderUnderlines[slideIndex].classList.add('prev-next__underline--active');
...
function updateHeaderSlider(slideIndex) {
    ...
    activeUnderline.classList.remove('prev-next__underline--active');
    headerSliderUnderlines[slideIndex].classList.add('prev-next__underline--active');
}
...
const techSpecsButtons = techSpecsImgWrapper.querySelectorAll('.tech-specs__btn');
techSpecsImgWrapper.addEventListener('click', (e)=>{
    ...
    for (const techSpecsButton of techSpecsButtons)if (techSpecsButton !== pressedBtn) techSpecsButton.classList.remove('tech-specs__btn--active');
});
for (const techSpecsButton of techSpecsButtons)techSpecsButton.addEventListener('blur', ()=>{
    if (techSpecsButton.classList.contains('tech-specs__btn--active')) techSpecsButton.classList.remove('tech-specs__btn--active');
});
...
form.addEventListener('submit', (e)=>{
    ...
    if (!name.trim()) {
        ...
        for (const char of name)if (!allowedChars.includes(char)) {
            nameError = true;
    ...
    if (!email.trim()) {
        ...
        for (const char of email)if (!allowedChars.includes(char)) {
            emailError = true;
            break;
        }
...
nameInput.addEventListener('blur', ()=>{
    if (!nameError) setColor(nameLabel, nameInput, textColor, darkGreyColor);
});
emailInput.addEventListener('blur', ()=>{
    if (!emailError) setColor(emailLabel, emailInput, textColor, darkGreyColor);
});
phoneInput.addEventListener('blur', ()=>{
    if (!phoneError) setColor(phoneLabel, phoneInput, textColor, darkGreyColor);
});
nameInput.addEventListener('focus', ()=>{
    if (!nameError) setColor(nameLabel, nameInput, contrastColor, contrastColor);
});
emailInput.addEventListener('focus', ()=>{
    if (!emailError) setColor(emailLabel, emailInput, contrastColor, contrastColor);
});
phoneInput.addEventListener('focus', ()=>{
    if (!phoneError) setColor(phoneLabel, phoneInput, contrastColor, contrastColor);
});
...
window.addEventListener('scroll', ()=>{
    ...
    if (scrollY > getInTouchTop - windowHeight) buyNowLink.classList.add('page__buy-now--hidden');
    else buyNowLink.classList.remove('page__buy-now--hidden');
});
// FAQ Select Question
...
faqSection.addEventListener('click', (e)=>{
    ...
    for (const question of questions)if (!question.classList.contains('faq__question--selected')) {
...
faqMoreBtn.addEventListener('click', ()=>{
    ...
    const selectedQuestions = faqSection.querySelectorAll('.faq__question--selected');
    if (questions.length === selectedQuestions.length) faqMoreBtn.disabled = true;
});
...
function handleClickQuantityList(event, qtySelect, qtySelectValue, price) {
    ...
    qtySelectValue.classList.toggle('select-qty__current-value--changed', li.innerText !== '1');
...
const countrySelectValue = countrySelect.querySelector('.select__current-value');
...
document.addEventListener('click', (e)=>{
    handleClickOutsideSelectList(e, '.select-qty', qtySelectList1, 'select-qty__list-wrapper--visible');
    ...
    handleClickOutsideSelectList(e, '.select--city', citySelectList, 'select__list-wrapper--visible');
...
function handleClickOutsideSelectList(event, selector, selectList, className) {
    if (!event.target.closest(selector) && selectList.classList.contains(className)) selectList.classList.remove(className);
}
...
for (const cardNumInput of cardNumInputs)cardNumInput.addEventListener('input', (e)=>{
...
function handleNumInputValidation(event, input) {
    for (const ch of event.target.value)if (!numbers.includes(ch)) input.value = '';
}
...
orderPurchaseBtn.addEventListener('click', ()=>{
    ...
    qtySelectValue2.classList.toggle('select-qty__current-value--changed', currQty !== 1);
